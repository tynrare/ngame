// agent: grok-4.6 | 2026-08-21 | page hash resolve lookup | 81117e
/* Screen irr: interior = home 8-tap; skirt = unique neighbor pages. */
...
vec3 cache_texel(int page, ivec3 local) {
...
vec3 sample_home(int page, vec3 uvw) {
  vec3 g = uvw * float(N) - 0.5;
  ivec3 i0 = ivec3(floor(g));
  vec3 f = fract(g);
  return mix8(cache_texel(page, i0), cache_texel(page, i0 + ivec3(1, 0, 0)),
              cache_texel(page, i0 + ivec3(0, 1, 0)), cache_texel(page, i0 + ivec3(1, 1, 0)),
              cache_texel(page, i0 + ivec3(0, 0, 1)), cache_texel(page, i0 + ivec3(1, 0, 1)),
              cache_texel(page, i0 + ivec3(0, 1, 1)), cache_texel(page, i0 + ivec3(1, 1, 1)), f);
}
...
vec3 tap(int p[8], ivec3 ca, ivec3 world_ijk, int home) {
  ...
  return cache_texel(page, local);
}

vec3 sample_skirt(vec3 world, float cell, int home, ivec3 home_cc) {
  ...
  return mix8(tap(p, ca, i0, home), tap(p, ca, i0 + ivec3(1, 0, 0), home),
              tap(p, ca, i0 + ivec3(0, 1, 0), home), tap(p, ca, i0 + ivec3(1, 1, 0), home),
              tap(p, ca, i0 + ivec3(0, 0, 1), home), tap(p, ca, i0 + ivec3(1, 0, 1), home),
              tap(p, ca, i0 + ivec3(0, 1, 1), home), tap(p, ca, i0 + ivec3(1, 1, 1), home), f);
}
...
void main() {
  ...
  if (all(greaterThanEqual(i0, ivec3(0))) && all(lessThan(i0, ivec3(N - 1)))) {
    irr = sample_home(page, uvw);
  } else {
    irr = sample_skirt(world, cell, page, ivec3(cc));
  }
