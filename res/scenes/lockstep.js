...
Box.prototype.fixed_step = function (dt) {
  // Tick-indexed get_any_input under lockstep — OR of all peers for this sim tick.
  ...
  var ty = 0;
  var tz = 0;
  if (global.get_any_input(global.KEY_A)) tz += 1;
  if (global.get_any_input(global.KEY_D)) tz -= 1;
  if (global.get_any_input(global.KEY_W)) tx += 1;
  if (global.get_any_input(global.KEY_S)) tx -= 1;
  if (tx !== 0 || ty !== 0 || tz !== 0) {
    var s = 200.0;
