...
Performance north star: **probe work ∝ unique slots**, not screen pixels. Seed from **culled meshes**, not screen-pixel loops.
...
| Bad | Why | Prefer |
|-----|-----|--------|
| `LoadImageFromTexture` seed every frame | Sync stall; CPU O(seed) | Culled prim list from same frustum planes |
| Placement cost ∝ resolution | Violates discrete-probe budget | Deduped keys; work ∝ `slot_cap` |
| Full-clip always-cover | Fights camera motion; not surface-matched | Culled-surface cubic root + binary split |
...
| Phase | State |
|-------|--------|
| Cull + surface-hash binary base | **shipping** (debug probes tiles) |
| Further leaf splits / fill / resolve | **next** |
| B.3–B.5 hash spine | retained as storage |
| B.6–B.6.6 clip always-cover | **deprecated / refactor** |

**Ship path:** frustum cull → cubic root of culled AABB → binary-split ≥1 leaf/mesh → world hash; refine later.
