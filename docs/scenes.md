...
<!-- agent: grok-4.6 | 2026-08-21 | RC scenes point chunked grid | c330f0 -->
**Render (`render`):** omit / `"simple"` (default forward), `"gbuffer"` (debug G-buffer), `"rc"` (G-buffer + chunked world-grid RC: interval GI, sky/sun on last cascade; compose `N·L` is fill-light only; [north star](radiance-cascades-3d.md)).
