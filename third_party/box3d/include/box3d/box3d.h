...
/// Serialize the world at a step boundary into an owned byte blob (seed snapshot + geometry
/// registry in recording framing). Same ABI/layout as the recording format. Call at a step
/// boundary on an unlocked world. Caller frees with b3FreeSaveData. userData is not preserved.
/// @param worldId unlocked world to save
/// @param outData receives malloc'd bytes; never NULL on success
/// @param outSize receives byte count
/// @return true on success
B3_API bool b3World_Save( b3WorldId worldId, uint8_t** outData, int* outSize );

/// Free a blob returned by b3World_Save.
B3_API void b3FreeSaveData( uint8_t* data, int size );

/// Restore a b3World_Save blob into an existing unlocked shell world (created via b3CreateWorld).
/// Overwrites simulation state. Host must rebind body ids (userData cleared). Same-build layout
/// hash required.
/// @param worldId target shell world
/// @param data save blob
/// @param size byte count
/// @return true on success
B3_API bool b3World_Restore( b3WorldId worldId, const uint8_t* data, int size );
