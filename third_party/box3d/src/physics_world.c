...
// agent: composer-2.5 | 2026-07-30 | box3d world save restore | aa53d6
bool b3World_Save( b3WorldId worldId, uint8_t** outData, int* outSize )
{
	...
	*outSize = 0;
	...
	b3StartRecordingIntoBuffer( world, rec );
	b3StopRecordingInternal( world );

	int size = b3Recording_GetSize( rec );
	const uint8_t* src = b3Recording_GetData( rec );
	if ( size <= 0 || src == NULL )
	{
...
void b3FreeSaveData( uint8_t* data, int size )
{
	if ( data == NULL || size <= 0 )
	{
		return;
