...
// agent: composer-2.5 | 2026-07-30 | box3d world save restore | b91ffe
bool b3World_Restore( b3WorldId worldId, const uint8_t* data, int size )
{
	if ( data == NULL || size < (int)sizeof( b3RecHeader ) )
	{
		return false;
	...
	if ( hdr.pointerWidth != (uint8_t)sizeof( void* ) || hdr.bigEndian != 0 || hdr.snapshotSize == 0 )
	{
		return false;
	}
	...
	if ( !b3RecLoadSlots( &rdr, data, size, hdr.registryOffset, hdr.registryByteCount ) )
	{
		return false;
	}
	...
	int snapStart = (int)sizeof( b3RecHeader );
	int snapSize = (int)hdr.snapshotSize;
	bool ok = b3DeserializeIntoShell( data + snapStart, snapSize, world, &rdr );

	b3RecFreeSlots( rdr.slots, rdr.slotCount );
	if ( rdr.tags != NULL )
	{
		b3Free( rdr.tags, (size_t)rdr.tagCapacity * sizeof( b3RecTag ) );
	}
	if ( rdr.tagMap != NULL )
	{
		b3RecTagLookup_cleanup( (b3RecTagLookup*)rdr.tagMap );
		b3Free( rdr.tagMap, sizeof( b3RecTagLookup ) );
	}
