...
bool mod_scene_graph_sample_draw_pose(const NgSceneInst *inst, double now, float delay_s,
  ...
  /* Soft-skipped echoes leave samples stale; local author must draw live. */
  if (inst->prefer_live_draw || inst->sample_count == 0) {
    return false;
  }
  ...
  /* Euler lerp (shortest arc). Quat nlerp + asin-middle broke yaw in rot[1]. */
  {
    const float pi = 3.14159265359f;
    const float tau = 6.28318530718f;
    for (int i = 0; i < 3; i++) {
      float d = b->rot[i] - a->rot[i];
      if (d == d && d < 1e20f && d > -1e20f) {
        while (d > pi) {
          d -= tau;
        }
        while (d < -pi) {
